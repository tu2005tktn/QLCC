...
<body>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="${pageContext.request.contextPath}/parking/list" class="btn btn-sm btn-outline-secondary me-2">
            ...
            <div class="btn-group">
                <c:if test="${(registration.requesterId == sessionScope.user.userId || sessionScope.user.role.roleName eq 'ADMIN' || sessionScope.user.role.roleName eq 'MANAGER') && registration.status eq 'Đang hoạt động'}">
                    <a href="${pageContext.request.contextPath}/parking/update/${registration.parkingId}" class="btn btn-sm btn-warning me-2">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            <th>Vị trí đỗ xe:</th>
                            <td>${registration.parkingSlot ? registration.parkingSlot : 'Chưa xác định'}</td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <c:if test="${sessionScope.user.role.roleName eq 'ADMIN' || sessionScope.user.role.roleName eq 'MANAGER'}">
                <div class="card mb-4">
                    <div class="card-header bg-secondary text-white">
                        <h5 class="mb-0"><i class="fas fa-cog me-2"></i>Quản lý</h5>
                    </div>
                    <div class="card-body">
                        <form id="updateSlotForm" class="mb-3">
                            <div class="input-group">
                                <input type="text" class="form-control" id="parkingSlot" placeholder="Nhập vị trí đỗ xe" value="${registration.parkingSlot}">
                                <button class="btn btn-outline-primary" type="button" onclick="updateParkingSlot()">Cập nhật</button>
                            </div>
                        </form>
                        
                        <div class="d-grid gap-2">
                            <button type="button" class="btn btn-outline-danger" onclick="if(confirm('Bạn có chắc muốn hủy đăng ký này?')) window.location.href='${pageContext.request.contextPath}/parking/cancel/${registration.parkingId}'">
                                <i class="fas fa-ban me-1"></i> Hủy đăng ký
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function updateParkingSlot() {
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/parking/update-slot/${registration.parkingId}',
                type: 'POST',
                data: {
                    parkingSlot: parkingSlot
                },
                success: function(response) {
                    alert('Cập nhật vị trí đỗ xe thành công!');
